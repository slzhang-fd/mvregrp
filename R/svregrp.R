...
svregrp_Gibbs <- function(Y_star, x_covs, z_covs,
  ...
  for(iter in 1:max_steps){
    ...
    if(calcu_DIC){
      Dtheta_all[iter] <- calcu_Deviance(Y_star, x_covs, z_covs, i_ind, hit_ind, 
      ...
      if(iter>burn_in){
        ...
        mean_coeffs_mean <- (iter-burn_in-1.0) / (iter-burn_in) * mean_coeffs_mean + 1.0 / (iter-burn_in) * mean_coeffs
        corr_coeffs_mean <- (iter-burn_in-1.0) / (iter-burn_in) * corr_coeffs_mean + 1.0 / (iter-burn_in) * corr_coeffs
        ...
        Dtheta_mean <- (iter-burn_in-1.0) / (iter-burn_in) * Dtheta_mean + 1.0 / (iter-burn_in) * Dtheta_all[iter]
      }
    }
    if(!corr_vs_diag){
      if(iter %% 100 == 0)
  ...
  DIC_value <- 2 * Dtheta_mean - calcu_Deviance(Y_star, x_covs, z_covs, i_ind, hit_ind, sh_len,
...
svregrp_Gibbs_area <- function(Y_star, x_covs, z_covs,
  ...
  for(iter in 1:max_steps){
    ...
    U_all <- sample_lv_ge(U_all, sigma2_e_inv, sigma2_u_inv, u_len,
                          rowsum(Y_star - Xbeta - VH_all[hit_ind,] - W_all[area_ind,],
                                 i_ind, reorder = T))
    # sample U
    W_all <- sample_lv_ge(W_all, sigma2_e_inv, sigma2_w_inv, area_len,
    ...
    VH_all <- sample_lv_grp(VH_all, sigma2_e_inv, sigma2_v_inv,
                            ...
                            rowsum(Y_star - Xbeta - U_all[i_ind,] - W_all[area_ind,],
                                   hit_ind, reorder = T))
    # update parameters
    params <- sample_params_he_area(x_covs, z_covs, XtX, Y_star, U_all, VH_all, W_all,
                               mean_coeffs, corr_coeffs,
                               sigma2_e, sigma2_u, sigma2_v, 
    ...
    if(verbose) 
      cat(if (iter == max_steps) '\n' else '\r')
    if(calcu_DIC){
      Dtheta_all[iter] <- calcu_Deviance_area(Y_star, x_covs, z_covs, i_ind, area_ind, hit_ind, 
                                         sh_len, mean_coeffs, corr_coeffs, 
  ...
  DIC_value <- 2 * Dtheta_mean - calcu_Deviance_area(Y_star, x_covs, z_covs, i_ind, area_ind, hit_ind, sh_len,
                                                mean_coeffs_mean, corr_coeffs_mean,

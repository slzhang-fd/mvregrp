...
arma::mat generateSymmetricMatrix(const arma::vec &offDiagonal, int n) {
  ...
  arma::mat symmetricMatrix =
      arma::symmatl(A); // create a full symmetric matrix from the sparse matrix
...
double my_dmvnorm(arma::vec x, arma::mat Sigma) {
  ...
  arma::vec z = rooti * x;
  double loglik =
      -(double)xdim / 2.0 * log2pi - 0.5 * arma::accu(z % z) + rootisum;
  return loglik;
}
//' @export
// [[Rcpp::export]]
double calc_loglik(const arma::vec &sh_len_g1_indices,
                   ...
                   const arma::vec &Zbeta, const arma::vec &VH_all,
                   const double sigma2_v) {
  // loglik <- 0
...
//' @export
// [[Rcpp::export]]
void update_VH_multi(const arma::vec &sh_len_g1_indices,
                     const arma::vec &hit_len, const arma::vec &sh_h_mapper,
                     const arma::vec &z_sh_ind, const arma::vec &Zbeta,
                     arma::vec &VH_all, const arma::vec &temp,
  ...
  for (auto sh_loc : sh_len_g1_indices) {
    ...
    arma::mat Sigma_vs_inv =
        sigma2_v_inv *
        arma::inv_sympd(generateSymmetricMatrix(rho, h_loc.n_elem));
